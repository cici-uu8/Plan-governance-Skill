...
- 硬冲突：同 workstream 多个 active authoritative execution head；在 `strict_mainline` 模式下，只要同 workstream 出现多个 active execution head，即使都不是 authoritative，也必须报冲突并要求 pin、close 或 supersede 到单 head
...
- `strict_mainline` 模式下，同 workstream 多个 active execution head（即使 `authoritative=false`）也需要提示
...
| `语义层` | `v0.6` 本地实验 | explicit semantic command、registry-zero-relation soft edge、future embedding | 默认关闭，且必须带 provenance、confidence 和 relation_scope |
...
3. 进入 `v0.4 SQLite` 前
   ...
   2026-06-20 的外部用户式 dogfooding 进一步补上了两个确定性内核 P0：adoption report 必须诚实披露 scan scope 与 out-of-scope Markdown；`strict_mainline` 必须把多 active execution heads 视为 hard conflict，而不是要求用户先理解 `authoritative` 内部概念。
...
### 当前结论：语义层保留为实验，不继续加功能

真实仓库验证后，语义层的 raw overlap 候选从 42 条收缩到 1 条 registry-zero-relation 跨 workstream 边。这个结果证明语义层能提供少量增量价值，但不足以作为稳定公开能力继续扩边。

因此当前最稳妥的结论是：

- 保留 `semantic` 显式命令，作为本地实验和验证工具
- 维持默认关闭
- 不继续往语义层追加新功能，直到有新的外部用户和评测数据证明其价值
- 公开发布边界暂时停在确定性 registry / graph / SQLite / MCP 的清晰分层上
...
- 2026-06-20 外部用户式 dogfooding（`/Users/cici/anomaly_detection`）证明两类 supported-surface 风险必须优先修复后再继续：一是 adoption report 不能把“范围外没看”的 Markdown 伪装成 “0 skipped”；二是 `strict_mainline` 不能容忍同一 workstream 多个 active execution heads 但不报冲突。两项修复都已回到确定性内核与测试层，没有继续扩 experimental 功能面。
