...
| 14. Continue v0.5 MCP server | complete | `mcp` stdio server plus Codex install/uninstall/discovery, workspace-root-aware initialize, compact default context/impact output, expanded mode, and regression tests |
...
- MCP now has a first supported host path for Codex: `install`, `uninstall`, and `discover-mcp` manage one global Codex MCP entry, while the stdio server discovers the active repo from `rootUri` / `workspaceFolders` and falls back to explicit env override only when needed. `impact` and `context` default to compact ranked output; expanded mode remains available for full graph inspection.
