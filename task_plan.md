...
| 14. Continue v0.5 MCP server | complete | `mcp` stdio server plus Codex install/uninstall/discovery, workspace-root-aware initialize, unique child repo discovery from parent workspaces, per-tool `projectPath` / `repo_root` overrides, compact default context/impact output, expanded mode, and regression tests |
...
- MCP now has a first supported host path for Codex: `install`, `uninstall`, and `discover-mcp` manage one global Codex MCP entry, while the stdio server discovers the active repo from `rootUri` / `workspaceFolders`, can select a single nearest governed child repo from a parent workspace, and supports explicit per-tool `projectPath` / `repo_root` overrides. `impact` and `context` default to compact ranked output; expanded mode remains available for full graph inspection.
